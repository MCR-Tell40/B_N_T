...
{
  TFile *f1 = new TFile("0_root_longest_chain_full.root");

  TH1F *un_scramble_hist = (TH1F*)f1->Get("Un_Scramble_Longest_Chain");
  TH1F *new_scramble_hist = (TH1F*)f1->Get("New_Scramble_Longest_Chain");
  TH1F *old_scramble_hist = (TH1F*)f1->Get("Old_Scramble_Longest_Chain");

  TFile *f2 = new TFile("1_root_longest_chain_full.root");

  TH1F *un_scramble_hist_1 = (TH1F*)f2->Get("Un_Scramble_Longest_Chain");
  TH1F *new_scramble_hist_1 = (TH1F*)f2->Get("New_Scramble_Longest_Chain");
  TH1F *old_scramble_hist_1 = (TH1F*)f2->Get("Old_Scramble_Longest_Chain");
  ...
  un_scramble_hist_1->SetLineColor(1);
  ...
  // New Scramble
  new_scramble_hist_1->Draw("sames");
  new_scramble_hist_1->SetLineColor(2);
  new_scramble_hist_1->SetLineStyle(1);
  new_scramble_hist_1->SetLineWidth(3);
  //new_scramble_hist->Fit("gaus"); 
  //new_scramble_hist->GetFunction("gaus")->SetLineColor(2);

  //new_scramble_hist->GetYaxis()->SetLabelSize(0.02);
  new_scramble_hist_1->SetStats(0);
  
  
  // Old Scramble
  old_scramble_hist_1->Draw("sames");
  old_scramble_hist_1->SetLineColor(4);
  old_scramble_hist_1->SetLineStyle(7);
  old_scramble_hist_1->SetLineWidth(3);
  //old_scramble_hist->Fit("gaus"); 
  //old_scramble_hist->GetFunction("gaus")->SetLineColor(4); 
  old_scramble_hist_1->SetStats(0);
  
  // Legend
  leg_top = new TLegend(0.65,0.6,0.9,0.9);
  leg_top->AddEntry(un_scramble_hist_1,"Pre Scrambler","l");
  leg_top->AddEntry(new_scramble_hist_1,"New Scrambler","l");
  leg_top->AddEntry(old_scramble_hist_1,"Old Scrambler","l");
  leg_top->Draw();
